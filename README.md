...
- **Collections gallery** — three balanced columns of lookbook tiles. Runs of near-identical frames are collapsed into one tile that cycles through its frames on tap with a crossfade and an "i — k" counter (`components/GalleryStack.tsx`, grouping from `lib/gallery-layout.ts`), tiles ease in as they scroll into view (CSS scroll-driven animation, off under reduced motion), and on phones the columns collapse to one in shoot order.
