...
describe('import-lookbook script', () => {
  test('writes one jpeg per unique frame, named by frame number, with stale output removed', async () => {
    ...
    expect(fs.readdirSync(path.join(publicDir, 'lookbook')).sort()).toEqual(['0900.jpg', '1200.jpg', '1345.jpg']);
  });
  ...
  test('writes a manifest in frame order with the real output dimensions', async () => {
    await importLookbook({ sourceDir, publicDir, manifestPath, maxWidth: 200, maxHeight: 250, quality: 80 });
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    expect(manifest).toEqual({
      images: [
        { src: '/lookbook/0900.jpg', width: 200, height: 160 },
        { src: '/lookbook/1200.jpg', width: 200, height: 250 },
        { src: '/lookbook/1345.jpg', width: 200, height: 250 },
      ],
    });
  });
});
