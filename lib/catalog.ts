...
// trousers, shirts), which is the order /shop displays. Edit text here;
// images come from lib/looks.json (regenerate with
// `npm run import:looks -- <folder>`). Handles are URLs and are persisted in
// visitors' bags, so they stay fixed when titles change.
type Look = {
  look: number;
  handle: string;
  title: string;
