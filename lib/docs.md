...
- @/lib/gallery-layout.ts — `balanceColumns(images, count)`: shortest-column-first distribution of the lookbook into `count` columns, returning each image with its source index and a per-column `scale` (≤ 1) that equalises column heights. Pure and unit-tested; consumed only by @/app/collections/page.tsx.
