...
- `GalleryStack` mounts every frame of a stack in the DOM, so hidden frames still download once the tile is in view: for the committed shoot all 144 lookbook frames are fetched while only the 61 stacks' current frames are visible at once. The current-frame index is per-tile `useState`; it is not persisted and resets on navigation.
