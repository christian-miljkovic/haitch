...
- `AddToCart` — size `<select>` that disables sold-out variants (labels them `— SOLD OUT`), defaults to the first available size, and disables the button entirely when nothing is available. The product page only mounts it when the product has variants, which since the Shopify sync means the look matched a product in @/lib/shopify-products.json.
...
- The cart and checkout code paths are exercised by tests with a synthetic purchasable product. In the live UI they become reachable for a look as soon as `npm run sync:shopify` has matched it to a store product and the resulting @/lib/shopify-products.json is committed; with the snapshot still empty, "priced but not purchasable" is the state of every product page and grid tile.
