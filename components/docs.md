...
- `GalleryStack` — client component and the only interactive piece of `/collections`. It takes the frames of one stack plus the tile's `position` and renders every frame as an absolutely positioned `next/image` filling the tile (`object-fit: cover`, real `width`/`height` from the manifest); only the frame at the `current` index is visible (0.6s opacity crossfade) and the others carry `aria-hidden`. With more than one frame the tile is a `<button>` labelled "Lookbook image N, frame i of k. Show next frame" that advances and wraps on click, with a small "i — k" counter bottom-left in 10px tracked type using `mix-blend-mode: difference` so it reads on any photo. A single-frame stack renders as a plain `<div>` with no button semantics. Only the first frame of the first three tiles is `loading="eager"`.
...
- `GalleryStack` mounts every frame of a stack in the DOM, so each tile also reveals itself (rise + fade over 0.9s, cascaded by column) when an `IntersectionObserver` first sees it, and its photo drifts inside the tile on a CSS `view()` timeline where supported (`overflow: clip`, not `hidden`, so the tile is not its own scroll container); hidden frames still download once the tile is in view: for the committed shoot all 144 lookbook frames are fetched while only the 61 stacks' current frames are visible at once. The current-frame index is per-tile `useState`; it is not persisted and resets on navigation.
