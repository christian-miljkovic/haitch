...
- `ProductGrid` — server component; borderless tile grid where each tile stacks the first two product images for a CSS hover swap. The price span renders only when `p.price` is defined, which for the committed catalog is every product (prices come from the line sheet via @/lib/catalog.ts). The first four tiles use `loading="eager"`, the rest lazy-load.
...
- `ProductGrid`/`ProductGallery` (`/looks/...`), `GalleryStack`/`GalleryViewer` (`/lookbook/...`), and `NewsletterModal` (`NEWSLETTER_IMAGE.src`, also under `/lookbook/`) pass site-relative paths to `next/image`, so they go through Next's built-in optimizer; `BagDrawer`/`CheckoutFlow` may render legacy `cdn.shopify.com` images from bag lines persisted before the catalog switch, which is why that host remains allowlisted in @/next.config.ts.
- The cart and checkout code paths are fully exercised by tests with a synthetic purchasable product, but no product in the live catalog currently has variants, so they are unreachable from the UI until Shopify variants are supplied. Prices, by contrast, are live: every catalog product now shows one, so "priced but not purchasable" is the normal state of the product page and shop grid.
